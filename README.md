...
4. 🧩 **Add the NoMount module:** ReSukiSU Manager → **Modules → Install from storage** → select `00_NoMount-Module-…zip`.
...
- **Remove** — delete the NoMount module in ReSukiSU Manager and reboot, then flash a stock boot image (or take an OTA) to drop the custom kernel.
